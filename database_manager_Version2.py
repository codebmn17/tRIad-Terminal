...
class DatabaseManager:
    ...
    def execute_query(
        self, database_name: str, query: str, parameters: tuple = ()
    ) -> tuple[bool, Any]:
...
class QueryBuilder:
    ...
    @staticmethod
    def update(
        table_name: str, columns: dict[str, Any], where: dict[str, Any]
    ) -> tuple[str, list[Any]]:
        """
        ...
        if args.command == "list":
            ...
            if success:
                if isinstance(result, dict) and "columns" in result:
                    ...
                    # Non-select query
                    print(
                        f"Query executed successfully. Rows affected: {result.get('rowcount', 0)}"
                    )
            else:
            ...
            # Export to CSV
            success, message = db_manager.export_csv(args.database, args.table, args.file)
            ...
            # Backup database
            success, message = db_manager.backup_database(args.database, args.output)
