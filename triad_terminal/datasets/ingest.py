...
class IngestionPhase:
    ...
    def should_run(
        self, dataset: DatasetRecord, force: bool = False, skip_phases: list[str] | None = None
    ) -> bool:
            ...
            return [
                {
                    ...
                    "docstring": f"Function that returns {i}",
                }
            ...
            return [
                {
                    ...
                    "description": f"Description of historical event {i}",
            ...
            return [
                {
                    ...
                    "company": f"Company {i}",
                    "document_type": "10-K",
                    "content": f"Financial filing content {i}",
                }
                for i in range(min(dataset.sample_size or 10, 10))
            ]
        else:
...
class IngestionPipeline:
    ...
    def __init__(self):
        self.phases = [DownloadPhase(), NormalizePhase(), EmbedPhase()]
    ...
    def ingest_dataset(
        self, dataset_id: str, force: bool = False, skip_phases: list[str] | None = None
    ) -> bool:
        ...
        thread = threading.Thread(
            target=self._ingest_dataset_thread, args=(dataset, force, skip_phases), daemon=True
        )
    ...
    def ingest_datasets_batch(
        self, dataset_ids: list[str], force: bool = False, skip_phases: list[str] | None = None
    ) -> int:
    ...
    def _ingest_dataset_thread(
        ...
        try:
            self._publish_event(
                "ingestion_started",
                {"dataset_id": dataset.id, "timestamp": datetime.utcnow().isoformat()},
            )
    ...
    def _publish_event(self, event_type: str, data: dict[str, Any]):
        ...
        try:
            self.event_manager.publish("datasets", {"type": event_type, "data": data})
        except Exception as e:
