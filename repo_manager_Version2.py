...
class GitHubAPI:
    ...
    def _make_request(
        self, method: str, url: str, data: Any = None, headers: dict[str, str] = None
    ) -> tuple[bool, Any]:
        ...
        try:
            ...
            request = urllib.request.Request(
                url=url, data=data_bytes, headers=headers, method=method
            )
    ...
    def list_repos(
        self, username: str = None, type: str = "all"
    ...
    def create_gist(
        self, description: str, files: dict[str, str], public: bool = False
    ) -> tuple[bool, dict[str, Any]]:
        """Create a new gist"""
        ...
        data = {"description": description, "public": public, "files": formatted_files}
...
class RepositoryManager:
    ...
    def create_repo(
        ...
        if remote:
            ...
            if not success:
                return (
                    False,
                    f"Failed to create GitHub repository: {response.get('message', 'Unknown error')}",
                )
    ...
    def create_branch(self, name: str, branch_name: str, checkout: bool = True) -> tuple[bool, str]:
        ...
        if checkout:
            checkout_success, _, checkout_error = GitCommand.run(["checkout", branch_name], path)

            if not checkout_success:
                return (
                    False,
                    checkout_error or f"Created branch but failed to checkout {branch_name}",
                )
    ...
    def list_branches(self, name: str) -> tuple[bool, list[dict[str, Any]]]:
        ...
        for line in stdout.splitlines():
            ...
            branches.append({"name": branch_name, "is_current": is_current, "is_remote": is_remote})
    ...
    def create_github_issue(
        self, name: str, title: str, body: str = "", labels: list[str] = None
    ) -> tuple[bool, dict[str, Any]]:
        ...
        success, response = self.github.create_issue(
            github_info["owner"], github_info["repo"], title, body, labels
        )
    ...
    def create_pull_request(
        self, name: str, title: str, head: str = None, base: str = "main", body: str = ""
    ) -> tuple[bool, dict[str, Any]]:
        ...
        success, response = self.github.create_pull_request(
            github_info["owner"], github_info["repo"], title, head, base, body
        )
...
def main():
    ...
    init_parser.add_argument(
        "--remote", action="store_true", help="Create remote GitHub repository"
    )
    init_parser.add_argument("--description", help="Repository description")
    init_parser.add_argument(
        "--private", action="store_true", help="Make GitHub repository private"
    )
    ...
    branch_parser.add_argument(
        "--no-checkout", action="store_true", help="Don't checkout the new branch"
    )
    ...
    checkout_parser.add_argument(
        "--create", "-b", action="store_true", help="Create and checkout the branch"
    )
    ...
    try:
        if args.command == "init":
            success, message = manager.create_repo(
                args.path, args.name, args.remote, args.description, args.private
            )

            print(f"{'✅' if success else '❌'} {message}")

        elif args.command == "clone":
            success, message = manager.clone_repo(args.url, args.dir, args.name)
            ...
            success, response = manager.create_github_issue(
                args.name, args.title, args.body, labels
            )
            ...
            if args.token:
                ...
                if success:
                    ...
                    print(
                        f"❌ Token validation failed: {user_info.get('message', 'Unknown error')}"
                    )
            else:
