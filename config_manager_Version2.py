    ...
    def __init__(self, config_dir: str, config_name: str = "config.yml"):
        ...
        self.schema = {
            "theme": {
                "type": str,
                "default": "matrix",
                "choices": ["matrix", "cyberpunk", "synthwave", "bloodmoon"],
            },
            "plugins_enabled": {"type": list, "default": ["git", "deployment", "api"]},
            "api_keys": {"type": dict, "default": {}},
            "user": {
                "type": dict,
                "default": {"name": os.environ.get("USER", "developer"), "email": ""},
            },
            "performance": {
                "type": dict,
                "default": {"animations": True, "parallel_operations": True, "cache_results": True},
            },
            "advanced": {
                "type": dict,
                "default": {
        ...
        for key, schema_item in self.schema.items():
            if key in loaded_config:
                ...
                if not isinstance(value, schema_item["type"]):
                    logger.warning(
                        f"Config value for '{key}' is wrong type. Expected {schema_item['type'].__name__}, got {type(value).__name__}"
                    )
                    result[key] = schema_item["default"]
                # Check choices if specified
                elif "choices" in schema_item and value not in schema_item["choices"]:
                    logger.warning(
                        f"Config value for '{key}' must be one of {schema_item['choices']}"
                    )
                    result[key] = schema_item["default"]
                else:
                    result[key] = value
