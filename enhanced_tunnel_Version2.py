...
class TunnelManager:
    ...
    def get_tunnels(self) -> dict[str, TunnelConfig]:
        ...
        if os.path.exists(self.tunnels_file):
            try:
                with open(self.tunnels_file) as f:
        ...
        try:
            ...
            cmd.extend(
                [
                    ...
                    "-o",
                    "ExitOnForwardFailure=yes",  # Exit if forwarding fails
                    "-o",
                    "ServerAliveInterval=60",  # Keep-alive packets every 60 seconds
                    "-o",
                    "ServerAliveCountMax=3",  # Allow 3 missed keep-alives before disconnect
                ]
            )
            ...
            if self.config.auto_restart:
                ...
                self.auto_restart_thread = threading.Thread(
                    target=self._monitor_and_restart, daemon=True
                )
...
class TunnelService:
    ...
    def start_tunnel(self, name: str) -> dict[str, Any]:
        ...
        if name not in tunnels:
            return {"success": False, "error": f"Tunnel '{name}' not found"}
        ...
        if name in self.active_tunnels:
            ...
            if tunnel_process.running:
                return {"success": True, "message": f"Tunnel '{name}' is already running"}
        else:
        ...
        if tunnel_process.start():
            return {"success": True, "message": f"Tunnel '{name}' started successfully"}
        else:
            return {"success": False, "error": f"Failed to start tunnel '{name}'"}

    def stop_tunnel(self, name: str) -> dict[str, Any]:
        """Stop a tunnel by name"""
        if name not in self.active_tunnels:
            return {"success": False, "error": f"Tunnel '{name}' is not running"}

        tunnel_process = self.active_tunnels[name]

        if tunnel_process.stop():
            del self.active_tunnels[name]
            return {"success": True, "message": f"Tunnel '{name}' stopped successfully"}
        else:
            return {"success": False, "error": f"Failed to stop tunnel '{name}'"}

    def get_tunnel_status(self, name: str = None) -> dict[str, Any] | list[dict[str, Any]]:
...
class MultiplexerTunnel:
    ...
    def add_tunnel(
        self, local_port: int, remote_host: str, remote_port: int, reverse: bool = False
    ) -> bool:
        """Add a tunnel to the multiplexed connection"""
        if (
            not self.control_path
            or not self.control_process
            or self.control_process.poll() is not None
        ):
        ...
        try:
            ...
            cmd.extend(
                [
                    ...
                    "-N",  # Don't execute remote command
                    "-O",
                    "forward",  # Control command to add a forward
                ]
            )
            ...
            if process.returncode == 0:
                ...
                logger.info(
                    f"Added {'reverse' if reverse else 'forward'} tunnel: {local_port} -> {remote_host}:{remote_port}"
                )
                return True
            else:
...
class JumpHostTunnel:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Jump host tunnel established: localhost:{local_port} -> {target_host}:{target_port}"
            )
            return True
