...
try:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            "assistant_ml", os.path.join(project_root, "agents", "learning", "assistant-ML.py")
        )
...
async def get_assistant_status() -> AssistantStatus:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get assistant status: {str(e)}"
        ) from e
...
async def predict_command(request: PredictCommandRequest) -> PredictCommandResponse:
    ...
    try:
        ...
        suggestions = assistant.predict_command(prefix=request.prefix)
        ...
        context_suggestions = assistant.predict_command(prefix=request.prefix)[
        ...
        raise HTTPException(status_code=500, detail=f"Command prediction failed: {str(e)}") from e
...
async def process_natural_language(request: NLRequest) -> NLResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Natural language processing failed: {str(e)}"
        ) from e
...
async def complete_code(request: CodeCompletionRequest) -> CodeCompletionResponse:
    ...
    try:
        ...
        suggestions = assistant.complete_code(
            code_context=request.code_context, language=request.language
        )
        ...
        raise HTTPException(status_code=500, detail=f"Code completion failed: {str(e)}") from e
...
async def provide_feedback(request: FeedbackRequest) -> FeedbackResponse:
    ...
    try:
        ...
        return FeedbackResponse(
            success=True, message="Feedback processed successfully", intent_guessed=intent_guessed
        )

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to process feedback: {str(e)}") from e
...
async def train_assistant(request: TrainRequest) -> TrainResponse:
    ...
    try:
        ...
        result = assistant.train_models(
            force=request.force, components=request.components or ["all"]
        )
...
async def get_schemas() -> SchemaResponse:
    ...
    try:
        ...
        return SchemaResponse(
            schemas=schema_data["assistant_schemas"], schema_info=schema_info, api_version="1.0.0"
        )
