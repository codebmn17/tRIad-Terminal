...
class VoiceManager:
    ...
    def _init_tts(self) -> None:
        ...
        try:
            ...
            # We'll create a wrapper to make it compatible with our interface
            class GTTSWrapper:
        ...
        def voice_assistant_thread():
            def on_trigger_detected(command_text):
                if not command_text:
                    ...
                    command_text = self.listen(timeout=5, phrase_time_limit=5)
                
                if command_text:
                    # Process the command
                    response = command_handler(command_text)
                    # Speak the response
                    if response:
                        self.speak(response)
    ...
    def _play_notification(self, sound_type: str = "notification") -> None:
        ...
        try:
            ...
            # Map of sound types to filenames
            sounds = {
            ...
            sound_file = os.path.join(self.config_dir, "sounds", sounds.get(sound_type, "notification.mp3"))
            ...
            if not os.path.exists(sound_file):
                ...
                # We'll need to generate a simple sound since we don't have one
    ...
    def _generate_notification_sound(self, sound_type: str, output_file: str) -> None:
        ...
        try:
            # Try using synthesizer if available
            ...
            if sound_type == "activation":
                ...
                chord = [synthesizer.generate_chord([440, 880], 0.1)]
            elif sound_type == "error":
                # Error sound: descending tone
                chord = [synthesizer.generate_chord([880, 440], 0.1)]
            elif sound_type == "success":
                # Success sound: pleasant chord
                chord = [synthesizer.generate_chord([523, 659, 784], 0.1)]
            else:
                # Default notification: simple beep
                chord = [synthesizer.generate_constant_wave(440.0, 0.1)]
