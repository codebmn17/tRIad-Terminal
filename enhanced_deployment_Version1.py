        ...
        try:
            ...
            if 'build_command' in config:
                ...
                if build_process.returncode != 0:
                    result['error'] = f"Build failed: {build_stderr.decode() if build_stderr else 'unknown error'}"
                    return result
        ...
        try:
            ...
            if aws_type == 's3':
                ...
                if zip_process.returncode != 0:
                    result['error'] = f"Failed to create deployment package: {zip_stderr.decode() if zip_stderr else 'unknown error'}"
                    return result
    ...
    async def deploy(self, project_dir: str, target_name: str) -> Dict[str, Any]:
        ...
        try:
            if target.type == 'vercel':
                ...
                result = {
                    'success': False,
                    'error': f"Unsupported deployment type: {target.type}"
                }
        except Exception as e:
        ...
        # Log the deployment
        self.manager.log_deployment(target_name, 
...
class DeploymentTemplates:
    ...
    @staticmethod
    def create_vercel_template(name: str, production: bool = False, team: Optional[str] = None) -> DeploymentTarget:
    ...
    @staticmethod
    def create_netlify_template(name: str, site_id: Optional[str] = None, 
                               production: bool = False,
                               publish_dir: str = "public") -> DeploymentTarget:
    ...
    @staticmethod
    def create_aws_s3_template(name: str, bucket: str, region: str = "us-east-1", 
                              public: bool = True) -> DeploymentTarget:
