    ...
    def matrix_rain(duration: float = 3.0) -> None:
        ...
        try:
            ...
            for i in range(width // 3):  # One stream every ~3 columns
                ...
                streams.append(
                    {"column": column, "speed": speed, "length": length, "head_pos": head_pos}
                )
                        ...
                        for i in range(stream["length"]):
                            ...
                            if 0 <= pos < height:
                                if i == 0:
                                    screen[pos][
                                        stream["column"]
                                    ] = f"{BRIGHT_GREEN}{random.choice(chars)}{RESET}"
                                else:
                                    screen[pos][
                                        stream["column"]
                                    ] = f"{GREEN}{random.choice(chars)}{RESET}"
...
class TerminalUI:
    ...
    def print_intro(self, skip_animation: bool = False) -> None:
        ...
        if HAS_RICH and self.console:
            ...
            info_text.append(
                "Welcome to the next generation of development environments",
            ...
            if HAS_COLORAMA:
                color_start = getattr(
                    colorama.Fore,
                    self.theme_manager.get_color("foreground").upper(),
                    colorama.Fore.WHITE,
                )
...
""",
    ...
    def create_dashboard_layout(self) -> Layout | None:
        ...
        layout["main"].split_row(Layout(name="sidebar", ratio=1), Layout(name="content", ratio=3))
