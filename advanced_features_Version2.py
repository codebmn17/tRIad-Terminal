...
class ResourceMonitor:
    ...
    def log_resource_usage():
        ...
        logger.debug(
            f"CPU: {info['cpu_percent']}% | Memory: {info['memory']['percent']}% | Disk: {info['disk']['percent']}%"
        )
...
class DevelopmentServer:
        ...
        try:
            if server_type == "node":
                if os.path.exists(os.path.join(self.project_dir, "package.json")):
                    if os.path.exists(
                        os.path.join(self.project_dir, "node_modules", ".bin", "vite")
                    ):
                        self.process = psutil.Popen(
                            ["npx", "vite", "--port", str(self.port)], cwd=self.project_dir
                        )
                    elif os.path.exists(
                        os.path.join(self.project_dir, "node_modules", ".bin", "next")
                    ):
                        self.process = psutil.Popen(
                            ["npx", "next", "dev", "-p", str(self.port)], cwd=self.project_dir
                        )
                    else:
                        self.process = psutil.Popen(
                            ["npx", "serve", "-l", str(self.port)], cwd=self.project_dir
                        )
                else:
                    self.process = psutil.Popen(
                        ["npx", "serve", "-l", str(self.port)], cwd=self.project_dir
                ...
                if os.path.exists(os.path.join(self.project_dir, "manage.py")):
                    self.process = psutil.Popen(
                        [sys.executable, "manage.py", "runserver", f"0.0.0.0:{self.port}"],
                    ...
                    self.process = psutil.Popen(
                        [sys.executable, "-m", "http.server", str(self.port)], cwd=self.project_dir
                    )

            elif server_type == "php":
                self.process = psutil.Popen(
                    ["php", "-S", f"0.0.0.0:{self.port}"], cwd=self.project_dir
                )
    ...
    def _detect_project_type(self) -> str:
        ...
        if any(
            os.path.exists(os.path.join(self.project_dir, f))
            for f in ["requirements.txt", "manage.py", "app.py"]
        ):
            return "python"
...
class CodeAnalyzer:
    ...
    def analyze_python_code(file_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            results["errors"].append(
                {"line": 0, "message": f"Analysis failed: {str(e)}", "code": "analysis-error"}
            )
...
"""
            ...
            if not os.path.exists(index_path):
                with open(index_path, "w") as f:
                    f.write(
                        """Welcome to Triad Terminal's documentation!
=====================================
...
"""
            ...
            subprocess.run(
                ["sphinx-apidoc", "-o", output_dir, project_dir, "--force", "--separate"],
                check=True,
            )
    ...
    def run_python_tests(project_dir: str) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                results["details"].append({"name": "pytest_output", "output": process.stdout})

            logger.info(
                f"Test results: {results['passed']} passed, {results['failed']} failed, {results['errors']} errors"
            )
            return results
            ...
            results["errors"] = 1
            results["details"].append({"name": "error", "error": str(e)})
            return results
