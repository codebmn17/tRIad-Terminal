...
class SecurityManager:
        ...
        if "locked_until" in user and user["locked_until"]:
            ...
            if locked_until > datetime.datetime.now():
                minutes_remaining = int(
                    (locked_until - datetime.datetime.now()).total_seconds() / 60
                )
    ...
    def _create_session(self, username: str) -> str:
        ...
        sessions[session_id] = {
            ...
            "expires_at": (
                ...
                + datetime.timedelta(minutes=self.settings.get("session_timeout_minutes", 30))
            ).isoformat(),
        }
    ...
    def setup_biometric(
        self, username: str, password: str, biometric_data: str
    ) -> tuple[bool, str]:
