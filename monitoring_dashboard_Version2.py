                ...
                for category in self.metrics_history:
                    if category in metrics:
                        ...
                        if len(self.metrics_history[category]) > self.history_length:
                            self.metrics_history[category] = self.metrics_history[category][-self.history_length:]
...
class ApplicationMonitor:
                    ...
                    try:
                        ...
                        if (process.info['name'] == process_name or 
                            (process.info['cmdline'] and 
                             any(process_name in cmd for cmd in process.info['cmdline']))):
    ...
    def get_application_metrics(self, name: Optional[str] = None) -> Union[Dict[str, Any], Dict[str, Dict[str, Any]]]:
        """Get metrics for a specific application or all applications"""
        if name:
            return self.metrics.get(name, {'status': 'unknown', 'error': 'Application not monitored'})
        else:
            return self.metrics
    
    def get_application_history(self, name: Optional[str] = None) -> Union[List[Dict[str, Any]], Dict[str, List[Dict[str, Any]]]]:
...
class DashboardServer:
    ...
    def __init__(self, system_monitor: SystemMonitor, app_monitor: ApplicationMonitor, host: str = 'localhost', port: int = 8050):
        if not HAS_DASH:
            raise ImportError("Dash is required for web dashboard. Install with: pip install dash plotly")
    ...
    def _setup_callbacks(self) -> None:
        ...
        def update_system_overview(_):
            ...
            return html.Div([
                html.Div([
                    html.H4("CPU"),
                    html.Div(f"{cpu_percent:.1f}%", style={
                        'fontSize': '24px', 
                        'color': '#ff4136' if cpu_percent > 80 else ('#ff851b' if cpu_percent > 50 else '#2ecc40')
                    })
                ], className='four columns'),
                
                html.Div([
                    html.H4("Memory"),
                    html.Div(f"{memory_percent:.1f}%", style={
                        'fontSize': '24px', 
                        'color': '#ff4136' if memory_percent > 80 else ('#ff851b' if memory_percent > 50 else '#2ecc40')
                    })
                ], className='four columns'),
                
                html.Div([
                    html.H4("Processes"),
                    html.Div(f"{metrics.get('processes', {}).get('count', 0)}", style={'fontSize': '24px'})
                ], className='four columns')
            ], className='row')
        ...
        def update_memory_graph(_):
            ...
            swap_percent = [item['data']['swap']['percent'] for item in memory_history if 'swap' in item['data']]
        ...
        def update_applications_table(_):
            ...
            return html.Table([
                ...
                html.Tbody([
                    html.Tr([
                        html.Td(name),
                        html.Td(app['status'], style={
                            'color': '#ff4136' if app['status'] in ['error', 'not_running'] else 
                                     '#ffdc00' if app['status'] in ['unhealthy', 'inactive'] else 
                                     '#2ecc40'
                        }),
...
class TerminalDashboard:
    ...
    def __init__(self, system_monitor: SystemMonitor, app_monitor: ApplicationMonitor):
        if not HAS_RICH:
            raise ImportError("Rich is required for terminal dashboard. Install with: pip install rich")
    ...
    def _generate_layout(self) -> Layout:
        ...
        if not app_metrics:
            layout["apps"].update(Panel("No applications being monitored", title="Applications", border_style="red"))
        else:
