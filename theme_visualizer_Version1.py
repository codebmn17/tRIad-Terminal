...
class ThemePreview:
    ...
    def create_image_preview(
        theme: dict[str, Any], output_path: str, size: tuple[int, int] = (800, 600)
    ) -> bool:
        ...
        try:
            ...
            draw.rectangle(
                [(20, content_y), (20 + term_width, content_y + term_height)],
            ...
            for i, (name, color) in enumerate(color_items):
                ...
                draw.text(
                    (x + 5, swatch_y + swatch_height + 10), name, fill=fg_color, font=font_small
                )
            ...
            draw.text(
                (20, footer_y + 20),
                "Triad Terminal - The next generation development environment",
                fill=bg_color,
                font=font_medium,
            )
    ...
    def print_terminal_preview(theme: dict[str, Any]) -> None:
        ...
        if HAS_RICH:
            ...
            for name, color in colors.items():
                swatches.append(
                    Panel(f"[{color}]■■■■■[/]\n[white]{name}[/]", width=16, style=f"on {bg_color}")
                )
...
def main() -> None:
    ...
    generate_parser.add_argument(
        "--save", "-s", action="store_true", help="Save the generated theme"
    )
    ...
    if args.command == "preview":
        ...
        for theme_id, theme_data in themes.items():
            print(
                f"  - {theme_id}: {theme_data.get('name', 'Unnamed')} - {theme_data.get('description', '')}"
            )
