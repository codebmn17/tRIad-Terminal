...
class ASCIIArtGenerator:
    ...
    @staticmethod
    def create_banner(
        text: str, style: str = "standard", width: int = 50, padding: int = 1
    ) -> list[str]:
        """Create a banner with specified style"""
        style_chars = ASCIIArtGenerator.BANNER_STYLES.get(
            style, ASCIIArtGenerator.BANNER_STYLES["standard"]
        )
        ...
        # Top border
        banner.append(
            style_chars["top_left"]
            + style_chars["horizontal"] * (width - 2)
            + style_chars["top_right"]
        )
        ...
        # Bottom border
        banner.append(
            style_chars["bottom_left"]
            + style_chars["horizontal"] * (width - 2)
            + style_chars["bottom_right"]
        )
    ...
    def create_header(text: str, font: str = "standard") -> list[str]:
        ...
        # Get the font
        font_data = ASCIIArtGenerator.FIGLET_FONTS.get(
            font, ASCIIArtGenerator.FIGLET_FONTS["standard"]
        )
    ...
    @staticmethod
    def combine_art(
        art1: list[str], art2: list[str], horizontal: bool = True, spacing: int = 2
    ) -> list[str]:
    ...
    def create_logo(text: str, style: str = "standard") -> list[str]:
        ...
        banner_style = ASCIIArtGenerator.BANNER_STYLES.get(
            style, ASCIIArtGenerator.BANNER_STYLES["standard"]
        )
        ...
        # Top border
        logo.append(
            banner_style["top_left"]
            + banner_style["horizontal"] * (banner_width - 2)
            + banner_style["top_right"]
        )

        # Padding above header
        for _ in range(padding):
            logo.append(
                banner_style["vertical"] + " " * (banner_width - 2) + banner_style["vertical"]
            )
        ...
        for line in header:
            ...
            logo_line += (
                " " * (banner_width - 2 - len(line) - padding_spaces) + banner_style["vertical"]
            )
        ...
        # Bottom border
        logo.append(
            banner_style["bottom_left"]
            + banner_style["horizontal"] * (banner_width - 2)
            + banner_style["bottom_right"]
        )
...
def main() -> None:
    ...
    banner_parser.add_argument(
        ...
        "-s",
        default="standard",
        choices=list(ASCIIArtGenerator.BANNER_STYLES.keys()),
        help="Banner style",
    )
    ...
    header_parser.add_argument(
        ...
        "-f",
        default="standard",
        choices=list(ASCIIArtGenerator.FIGLET_FONTS.keys()),
        help="Header font",
    )
    ...
    art_parser.add_argument(
        ...
        "-a",
        action="append",
        choices=list(ASCIIArtGenerator.SMALL_ART.keys()),
