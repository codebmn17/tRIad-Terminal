...
class CriticAgent(Agent):
    ...
    async def handle(self, msg: Message) -> None:
        ...
        if msg.sender != self.name and "review" in msg.content.lower():
            await self.send(
                msg.room, f"Let me analyze that: {msg.content[:100]}...", role="assistant"
            )
    ...
    async def handle(self, msg: Message) -> None:
        ...
        if msg.role != "user" and not any(
            ...
            for word in ["execute", "run", "delete", "install", "download"]
        ):
            return
        cautions = [
        ...
        await self.say(
            msg.room,
            safe_md(f"Cautions for: {msg.brief()}\n\n{text}"),
            meta={"icon": self.role.icon},
        )
