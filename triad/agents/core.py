...
class Agent:
    ...
    async def send(
        ...
        *,
        ...
        if self._router:
            msg = Message(room=room, sender=self.name, content=content, role=role, meta=meta or {})
...
class Router:
    ...
    async def post(self, msg: Message) -> None:
        ...
        await asyncio.gather(*[agent.handle(msg) for agent in agents], return_exceptions=True)
...
class Agent:
    ...
    async def say(
        ...
        *,
        ...
        await self._router.post(
            Message(
                room=room,
                sender=self.name,
                content=content,
                role=role or self.role.name,
                meta=meta or {},
            )
        )
