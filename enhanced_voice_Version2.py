...
class EnhancedVoice:
    ...
    def __init__(self, config_dir: str = "~/.triad/voice"):
        ...
        self.default_config = {
            ...
            "api_keys": {"azure": "", "elevenlabs": ""},  # API keys for cloud services
    ...
    def _initialize_engines(self) -> None:
        ...
        try:
            ...
            for voice in voices:
                ...
                if (
                    voice_type != "default"
                    and voice_type.lower() in voice_name
                    or gender == "male"
                    and ("male" in voice_name or "david" in voice_name or "george" in voice_name)
                    or gender == "female"
                    and ("female" in voice_name or "zira" in voice_name or "heera" in voice_name)
                ):
            ...
            self.tts_engines["pyttsx3"] = {"engine": engine, "type": "offline", "quality": "medium"}
        ...
        try:
            ...
            self.tts_engines["gtts"] = {
                ...
                "type": "online",
                "quality": "high",
                "dependencies": {"gtts": gTTS, "playsound": playsound},
            }
        ...
        try:
            result = subprocess.run(
                ["espeak", "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
        ...
        try:
            result = subprocess.run(
                ["mimic3", "--version"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            if result.returncode == 0:
        ...
        if self.config["api_keys"]["azure"]:
            self.tts_engines["azure"] = {"engine": "azure", "type": "online", "quality": "high"}
    ...
    def _speak_with_engine(self, text: str, engine_name: str | None = None) -> None:
        ...
        try:
            ...
            if engine_name == "pyttsx3":
                ...
                cmd.extend(
                    [
                        "-a",
                        str(int(self.config["volume"] * 100)),
                        "-s",
                        str(rate),
                        "-p",
                        str(int(50 * self.config["pitch"])),
                        text,
                    ]
                )
                ...
                cmd = [
                    ...
                    voice,
                    "--length-scale",
                    str(1.0 / self.config["speaking_rate"]),
                    text,
                ]
    ...
    def speak(self, text: str, blocking: bool = False, engine: str = None) -> None:
        ...
        # Add to speech queue
        self.speech_queue.put({"text": text, "engine": engine, "blocking": blocking})
    ...
    def test_voice(
        self,
        text: str = "Hello, I am the Triad Terminal voice assistant. How can I help you today?",
    ) -> None:
